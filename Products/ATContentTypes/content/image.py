...
class ATImage(ATCTFileContent, ATCTImageTransform):
    ...
    def setImage(self, value, refresh_exif=True, **kwargs):
        ...
        self._setImageContent(value, **kwargs)
    
    def _setImageContent(self, value, **kwargs):
        """Temporary proc that will be factored away or renamed or something"""
        ...
        if self._isIDAutoGenerated(self.getId()):
            ...
            if request_id and not self._isIDAutoGenerated(request_id):
                # Request contains an ID explicitly set by the user; use that.
                pass
            elif clean_filename and (clean_filename == title or not title):
                # TODO: For the JS to be able to put the clean_filename into the Title field, add an AJAX call that returns the result of clean_filename().
                # If Title is blank or Title is the uploaded file name, use the uploaded file name as my ID.
            ...
            # Else, fall through to BaseObject._renameAfterCreation(), which Archetypes calls after all the fields have been set. It will rename me based on my Title iff my ID looks autogenerated (which it does if we don't setId() here).
